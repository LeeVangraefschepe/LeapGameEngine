...
{
	OnSimulationEvent.Notify(SimulationEvent{ SimulationEventType::OnCollisionEnter, s0, s1 });
	...
	bool /*objectRemoved*/)
{
	...
	OnSimulationEvent.Notify(SimulationEvent{ SimulationEventType::OnCollissionExit, pair.pShape0, pair.pShape1 });
...
void leap::physics::PhysXSimulationFilterCallback::NotifyStayingPairs()
{
	for (const auto& collisionPair : m_Collisions)
	{
		...
		OnSimulationEvent.Notify(SimulationEvent{ SimulationEventType::OnCollisionStay, pair.pShape0, pair.pShape1 });
	}
}