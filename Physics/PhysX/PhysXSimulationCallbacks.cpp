...
void leap::physics::PhysXSimulationCallbacks::onTrigger(physx::PxTriggerPair* pairs, physx::PxU32 count)
{
	for (unsigned int i{}; i < count; ++i)
	{
		...
		{	
			OnSimulationEventDelegate.Invoke(SimulationEvent{ SimulationEventType::OnTriggerEnter, pair.triggerShape, pair.otherShape });
		...
		case physx::PxPairFlag::Enum::eNOTIFY_TOUCH_LOST:
		{
			...
			OnSimulationEventDelegate.Invoke(SimulationEvent{ SimulationEventType::OnTriggerExit, pairIt->pShape0, pairIt->pShape1 });
...
void leap::physics::PhysXSimulationCallbacks::NotifyStayingPairs()
{
	...
	{
		OnSimulationEventDelegate.Invoke(SimulationEvent{ SimulationEventType::OnTriggerStay, triggerPair.pShape0, triggerPair.pShape1 });
	}
}
