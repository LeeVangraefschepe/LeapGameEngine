...
void leap::physics::PhysXObject::Apply(const std::function<void(void*, const glm::vec3&, const glm::quat&)>& syncFunc)
{
	...
	syncFunc(m_pOwner, position, rotation);
}
...
void leap::physics::PhysXObject::UpdateObject(PhysXEngine* pEngine, IPhysicsScene* pScene)
{
	...
	if (m_pRigidbody)
	{
		...
		//static_cast<physx::PxRigidDynamic*>(m_pActor)->setLinearDamping(0.1f);
	}
	else
	{
