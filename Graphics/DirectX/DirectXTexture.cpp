...
leap::graphics::DirectXTexture::DirectXTexture(ID3D11Device* pDevice, const std::string& path)
{
	...
	if (FAILED(result)) throw std::runtime_error{ "Failed to create WIC factory when creating a texture" };
	...
	if (FAILED(result)) throw std::runtime_error{ "Failed to load image using the WIC factory" };
	...
	if (dxgiFormat == DXGI_FORMAT_UNKNOWN)
	{
		...
		if (compatibleWICFormat == GUID_WICPixelFormatDontCare) throw std::runtime_error{ "Image format is not supported by the engine" };
		...
		if (FAILED(result)) throw std::runtime_error{ "Failed to create a WIC format converter" };

		// Make sure we can convert to the dxgi compatible format
		BOOL canConvert{};
		result = pWICConverter->CanConvert(wicFormat, compatibleWICFormat, &canConvert);
		if (FAILED(result) || !canConvert) throw std::runtime_error{ "The engine is not able to convert image to supported image format. Image format is not supported by the engine" };
		...
		if (FAILED(result)) throw std::runtime_error{ "Failed to convert image to supported image format. The given image formate is not supported by the engine" };
		...
		if (FAILED(result)) throw std::runtime_error{ "Failed to copy pixeldata to bitmap" };
	}
	else
	{
	...
	if (FAILED(result)) throw std::runtime_error{ "Failed to create directX texture using the WIC frame" };
	...
	if (FAILED(result)) throw std::runtime_error{ "Failed to create shader resource view with the given texture" };
