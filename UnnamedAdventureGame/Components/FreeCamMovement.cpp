...
void unag::FreeCamMovement::Start()
{
	...
	auto command = std::make_unique<leap::LambdaCommand>([=]()
		{
			GetTransform()->Translate(-GetTransform()->GetRight() * 10.0f * leap::GameContext::GetInstance().GetTimer()->GetDeltaTime());
		});
	...
	command = std::make_unique<leap::LambdaCommand>([=]()
		{
			GetTransform()->Translate(GetTransform()->GetForward() * 10.0f * leap::GameContext::GetInstance().GetTimer()->GetDeltaTime());
		});
